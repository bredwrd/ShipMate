...
public class HtmlPrep {
	...
	/*
	 * takes an html file, converts to validated xml and saves it as
	 * <fileName>.xml
	 */
	public static void cleanHtmlFile(File htmlFile) {
		...
		try {
			...
			new PrettyXmlSerializer(props).writeToFile(tagNode,
					"/tmp/brian/canadaShipping/" + xmlFileName, "utf-8");
	...
	/*
	 * taking a <fileName>.<ext> filename, converts to and returns
	 * <fileName>.xml
	 */
	...
	// Sample code for HtmlCleaner. Leave in for now as a reference of the
	// library's abilities.
	/*
	 * // optionally find parts of the DOM or modify some nodes TagNode[]
	 * myNodes = node.getElementsByXXX(...); // and/or Object[] myNodes =
	 ...
	 * manipulation/traversal // serialize a node to a file, output stream, DOM,
	 * JDom... new XXXSerializer(props).writeXmlXXX(aNode, ...); myJDom = new
	 * JDomSerializer(props, true).createJDom(aNode); myDom = new
	 * DomSerializer(props, true).createDOM(aNode);
	 */
}